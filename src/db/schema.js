...
const { pgTable, uuid, text, timestamp, jsonb, boolean, integer } = require('drizzle-orm/pg-core');
 ...
 * Saved/Favorite Jobs
 */
const savedJobs = pgTable('saved_jobs', {
 ...
 */
const searchPresets = pgTable('search_presets', {
    id: uuid('id').primaryKey().defaultRandom(),
    userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
